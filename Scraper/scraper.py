...
def scrape_bestiary_table(url: str, base_url: str):
    headers = {}
...
def scrape_beast_info(url: str, session):
    ...
    ru_name, en_name = name_split[0].strip(), name_split[1].strip()
    ...
    li_items_no_class = card_article_body.find_all("li", class_="", recursive=False)
        ...
        for node in li_abilities:
            match node["title"]:
                case KeyAbilities.strength.value:
                    sp = [i for i in node.text.split(" ") if i != ""]
    ...
    for item in li_items_no_class:
        match item.find("strong").text:
            ...
            case KeyWords.senses.value:
                ...
                # Удаление значений в скобках - Чувства слепое зрение? 60 футов (слеп за его пределами), пассивное Восприятие 10
                sp_text = re.sub(r" \(.+\)", "", item.text)
                sp = sp_text.replace(KeyWords.senses.value, "").strip().split(",")
                for sense in sp:
            ...
            case KeyWords.languages.value:
                ...
                if SenseTypes.telepathy.value in item.text:
                    ...
                    if telepathy_index != -1:
                        try:
                            info_card.telepathy = int(sp[telepathy_index+1])
                        except ValueError:
